...
func TestOnlyCallOnceOnMultipleDepChanges(t *testing.T) {
	r := New()
	i := r.CreateInput(1)
	c1 := r.CreateCompute1(i, func(v int) int { return v + 1 })             // 4 2
	c2 := r.CreateCompute1(i, func(v int) int { return v - 1 })             // 2 0
	c3 := r.CreateCompute1(c2, func(v int) int { return v - 1 })            // 1 -1
	c4 := r.CreateCompute2(c1, c3, func(v1, v3 int) int { return v1 * v3 }) // 4 -2
	changed4 := 0
